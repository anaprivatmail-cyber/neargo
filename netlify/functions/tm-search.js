...
// Združeno iskanje: Supabase submissions + (opcijsko) Ticketmaster
...
function normalizeSB(obj) {
  ...
  const image = obj.imagePublicUrl || null; // če v prihodnje shraniš URL slike
...
export const handler = async (event) => {
  ...
  try {
    ...
    // 1) Središče iskanja – CITY ima prednost
    let center = null;
    if (city) {
    ...
    const keyOf = (e) => `${e.source}:${e.id || e.name || ''}`;
    const map = new Map();

    [...sbItems, ...tmResults].forEach(e => { if (matches(e)) map.set(keyOf(e), e); });
    let combined = Array.from(map.values());

    // sort po razdalji (če imamo center + koordinate)
    if (center) {
      combined.sort((a,b)=>{
        const da = (a.venue?.lat && a.venue?.lon) ? haversineKm(center,{lat:a.venue.lat, lon:a.venue.lon}) : Infinity;
        const db = (b.venue?.lat && b.venue?.lon) ? haversineKm(center,{lat:b.venue.lat, lon:b.venue.lon}) : Infinity;
        return da - db;
      });
    }
