...
exports.handler = async (event) => {
  ...
  if (!authedUser && !scannerOk) {
    ...
    // za UI tretiramo kot invalid (ni namena razkrivati razlogov)
    return json({ ok: false, status: "invalid", error: "unauthorized" }, 401);
  }
  ...
  if (!token) return json({ ok: false, status: "invalid", error: "missing_token" }, 400);
  ...
  if (!ticket) {
    ...
    return json({ ok: false, status: "invalid", error: "not_found" }, 404);
  }
  ...
  if ((process.env.ENFORCE_PROVIDER || "false") === "true" && ticket.event_id && authedUser) {
    ...
    if (ev?.provider_user_id && ev.provider_user_id !== authedUser.id) {
      ...
      return json({ ok: false, status: "invalid", error: "forbidden" }, 403);
    }
  }
  ...
  if (eventId && String(eventId) !== String(ticket.event_id || "")) {
    ...
    return json({ ok: false, status: "invalid", error: "wrong_event", forEvent: ticket.event_id }, 409);
  }
  ...
  if (ticket.status === "redeemed") {
    ...
    return json({ ok: true, status: "used", redeemed_at: ticket.redeemed_at });
  }
  ...
  return json({ ok: true, status: "valid", redeemed_at: updated.redeemed_at });
};
