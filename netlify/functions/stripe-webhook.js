...
async function createInvoicePdf({ seqNo, buyer, totalGross, base, tax, paidAt, sessionId }) {
  ...
  line(`DDV (22%): ${(tax/100).toFixed(2)} ${CURRENCY.toUpperCase()}`);
  line(`SKUPAJ: ${(totalGross/100).toFixed(2)} ${CURRENCY.toUpperCase()}`, 13, bold, rgb(0,0.45,0.25));
  y -= 10;
...
exports.handler = async (event) => {
  try {
    ...
    try {
      ...
      // Vrnemo 200, da Stripe ne retry-a v neskončnost, a zabeležimo napako
    ...
    const { error: insErr } = await supa
      ...
      .insert({
        event_id: eventId,
        type,
        display_benefit: displayBenefit || summarizeBenefit({benefitType,benefitValue,freebieText}),
    ...
    // DDV razrez iz GROSS
    const totalGross = (type === "coupon") ? 200 : (s.amount_total || 0);
    ...
    const pdfBytes = await createInvoicePdf({
      ...
      totalGross, base, tax, paidAt, sessionId: s.id
    });
    ...
    } catch (e) { console.error("Upload invoice error:", e?.message || e); }
    ...
    if (process.env.BREVO_API_KEY && customerEmail) {
        ...
        <div style="font-family:Arial,Helvetica,sans-serif;max-width:680px;margin:auto;border:1px solid #e3eef7;border-radius:12px;overflow:hidden">
          ...
          <div style="padding:18px 20px;color:#0b1b2b">
            ...
            <p style="margin:12px 0 6px">
              V priponki je tvoja ${type==="coupon" ? "<b>QR koda kupona</b>" : "<b>QR koda vstopnice</b>"}.
              QR kodo <b>hrani</b> — lahko jo pokažeš s telefona, jo fotografiraš ali natisneš.
            </p>
            <p style="margin:0 0 6px">Račun <b>${seq}</b> je priložen kot PDF.</p>
            <p style="margin:16px 0 0;color:#5b6b7b;font-size:13px">
              Če imaš vprašanja, piši na
              <a href="mailto:${SUPPORT_EMAIL}" style="color:${primary};font-weight:800">${SUPPORT_EMAIL}</a>.
            </p>
          </div>
        </div>`;
      ...
      email.subject     = type==="coupon" ? "Hvala za nakup – kupon" : "Hvala za nakup – vstopnica";
      email.htmlContent = html;
      email.attachment  = [
        { name: "qr.png",           content: qrPngBuffer.toString("base64") },
        { name: `Racun-${seq}.pdf`, content: Buffer.from(pdfBytes).toString("base64") }
      ];
      try { await brevoApi.sendTransacEmail(email); }
      catch (e) { console.error("Brevo send error:", e?.message || e); }
    }
