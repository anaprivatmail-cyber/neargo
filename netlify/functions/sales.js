...
export const handler = async (event) => {
  try {
    ...
    // preprosta agregacija
    const { data: total } = await supa.from("tickets").select("id", { count:"exact", head:true });
    const { data: red }   = await supa.from("tickets").select("id", { count:"exact", head:true }).eq("status","REDEEMED");

    return json({ ok:true, sold: total?.length ?? total?.count ?? 0, redeemed: red?.length ?? red?.count ?? 0 });
  } catch (e) {
