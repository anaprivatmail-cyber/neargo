...
function normalizeTM(ev) {
  try {
    ...
    const img = (ev.images || []).sort((a, b) => (b.width || 0) - (a.width || 0))[0]?.url || null;
    return {
      id: `tm_${ev.id}`,
      source: 'ticketmaster',
      name: ev.name || 'Dogodek',
      url: ev.url || null,
      images: img ? [img] : [],
...
async function fetchTicketmaster({ TM_KEY, center, radiusKm, query, category, size, page }) {
  ...
  if (size != null) u.searchParams.set('size', String(size));      // <= upoštevaj 0/false
  if (page != null) u.searchParams.set('page', String(page));      // <= upoštevaj 0
  ...
  try {
    ...
    if (category) {
      ...
      arr = arr.filter(e => (e.classifications?.[0]?.segment?.name || '').toLowerCase() === want);
    }
...
function normalizeEB(ev) {
  try {
    ...
    return {
      ...
      venue: {
        name: venue.name || '',
        address: [venue.address?.address_1, venue.address?.city, venue.address?.country]
          .filter(Boolean)
          .join(', '),
        lat: venue.latitude != null ? parseFloat(venue.latitude) : null,
        lon: venue.longitude != null ? parseFloat(venue.longitude) : null
      }
    };
  } catch {
    return null;
...
async function fetchEventbrite({ EB_TOKEN, center, radiusKm, query, category, size, page }) {
  ...
  u.searchParams.set('expand', 'venue,category,logo');              // <= pomembno za ev.venue
  if (query) u.searchParams.set('q', query);
  if (size != null) u.searchParams.set('page_size', String(size));
  if (page != null) u.searchParams.set('page', String(page + 1));   // EB je 1-based

  try {
    const r = await fetch(u.toString(), { headers: { Authorization: `Bearer ${EB_TOKEN}` } });
    if (!r.ok) return [];
    const data = await r.json().catch(() => null);
    let events = (data?.events || []).map(normalizeEB).filter(Boolean);
    const matches = makeMatcher({ query, category, center, radiusKm });
...
export async function fetchBigApis(opts) {
  ...
  const [tm, eb] = await Promise.all([
    fetchTicketmaster({ TM_KEY: TICKETMASTER_API_KEY, ...shared }).catch(() => []),
    fetchEventbrite ({ EB_TOKEN: EB_PRIVATE_TOKEN,     ...shared }).catch(() => [])
  ]);
