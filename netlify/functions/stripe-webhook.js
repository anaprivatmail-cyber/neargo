...
exports.handler = async (event) => {
  try {
    ...
    if (process.env.BREVO_API_KEY && customerEmail) {
      ...
      email.attachment  = [
        { name:"qr.png",        content: (await QRCode.toBuffer(redeemUrl,{type:"png",margin:1,width:512})).toString("base64") },
        { name:`Racun-${seq}.pdf`, content: Buffer.from(pdfBytes).toString("base64") }
      ];
      try { await brevoApi.sendTransacEmail(email); } catch (e) { console.error("Brevo send error:", e?.message || e); }
    }
