...
export const handler = async (event) => {
  ...
  if (missing.length) return json({ ok:false, error:'Manjlkajoča polja: ' + missing.join(', ') }, 400);
  ...
  try{
    ...
    try {
      if (BREVO_API_KEY && payload.organizerEmail) {
        ...
        const linkEdit  = `${DOMAIN}/edit.html?token=${editToken}`;
        const linkStats = `${DOMAIN}/org-stats.html?stats=${statsToken}`;

        // Lepe povezave ("tukaj"), brez besedila o “Po potrditvi …”
        const html = `
          <div style="font-family:Arial,Helvetica,sans-serif;max-width:680px;margin:auto;border:1px solid #e3eef7;border-radius:12px;padding:16px">
            ...
            <p><b>Uredi dogodek:</b> <a href="${linkEdit}">tukaj</a></p>
            <p><b>Statistika prodaje/unovčitev:</b> <a href="${linkStats}">tukaj</a></p>
