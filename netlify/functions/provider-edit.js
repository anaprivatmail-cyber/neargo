...
export const handler = async (event) => {
    ...
    try{
      ...
      const form = `
        ...
        <form id="f">
          ...
          <div class="featBox">
            <label class="chk"><input type="checkbox" id="featured" name="featured" ${isFeatured?'checked':''}> Izpostavi (7 dni) – 10 €</label>
            ${featUntil ? `<div class="muted">Trenutno aktivno do: <b>${featUntil}</b></div>` : ''}
            <div class="warn">Če izklopiš izpostavitev ali jo prekličeš, se <b>plačilo ne povrne</b>.</div>
        ...
        <script>
          ...
          cancelBtn.onclick=()=>{
            if(confirm('Preklic izpostavitve: že plačani znesek se NE povrne. Nadaljujem?')){
          ...
          document.getElementById('save').onclick=async()=>{
            ...
            const fd=new FormData(f);
            // FormData ne pošlje unchecked checkboxa – ročno dodamo vrednost
            if(!featuredEl.checked) fd.set('featured','false');
            const patch=Object.fromEntries(fd.entries());

            // če je izpostavitev izklopljena, poskrbimo da se na strežniku pobriše featuredUntil
  ...
  if (event.httpMethod === 'POST'){
    try{
      ...
      // če izklopi izpostavitev, izbriši featuredUntil
