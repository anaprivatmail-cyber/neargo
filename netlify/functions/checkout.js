...
// vedno ≥ 0,50 € (50 centov)
const COUPON_PRICE_CENTS = Math.max(50, Number(process.env.COUPON_PRICE_CENTS || 200)); // 2,00 €
...
export const handler = async (event) => {
  try {
    ...
    // (novo) client_reference_id za lažje iskanje v Stripe
    if (metadata.event_id) form.set("client_reference_id", String(metadata.event_id));
    ...
    if (type === "coupon") {
      ...
      // (novo) lep opis v Payment Intent
      form.set("payment_intent_data[description]", name);
      ...
      // (novo) opis – prvi line item
      form.set("payment_intent_data[description]", items[0].name || "Vstopnica");
    }
    ...
    // (novo) Idempotentnost – varno proti dvojnim klikom/ponovnim poskusom
