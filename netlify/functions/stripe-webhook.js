...
async function createInvoicePdf({ seqNo, buyer, totalGross, base, tax, paidAt, sessionId }) {
  ...
  const regBytes  = fs.readFileSync(FONT_REG_TTF);
  const boldBytes = fs.readFileSync(FONT_BOLD_TTF);
  ...
  const line = (txt, size=11, f=fontReg, color=rgb(0,0,0))=>{
  ...
  line(`${buyer.email || ""}`, 11, fontReg);
  y -= 6;

  // zneski
  line("Obračun:", 12, fontBold);
  line(`Osnova: ${(base/100).toFixed(2)} ${CURRENCY.toUpperCase()}`);
  line(`DDV (${TAX_RATE}%): ${(tax/100).toFixed(2)} ${CURRENCY.toUpperCase()}`);
  line(`SKUPAJ: ${(totalGross/100).toFixed(2)} ${CURRENCY.toUpperCase()}`, 13, fontBold, rgb(0,0.45,0.25));
  ...
  y -= 8;
  line(`Stripe session: ${sessionId}`, 9, fontReg, rgb(0.4,0.4,0.4));
...
exports.handler = async (event) => {
  try {
    ...
    // zneski: kupon vedno 2,00 € (200 centov), vstopnica s.amount_total
