...
async function createInvoicePdf({ seqNo, buyer, items, subtotal, taxRate, taxAmount, total, paidAt, sessionId }) {
  ...
  // path do TTF pisav relativno na to datoteko
  const __dirname = path.dirname(fileURLToPath(import.meta.url));
  const fontRegBytes  = fs.readFileSync(path.join(__dirname, "fonts/NotoSans-Regular.ttf"));
  ...
  return Buffer.from(bytes);                    // ⬅️ zagotovimo Buffer
}
  ...
  try {
    ...
    const { data: ticketIns, error: ticketErr } = await supa
      ...
      .insert({
        ...
        token,
        code: token, // varnost pred NOT NULL (če ga DB zahteva)
        status: "issued",
