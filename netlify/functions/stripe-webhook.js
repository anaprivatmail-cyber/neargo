...
// Ob checkout.session.completed:
// - zapiše ticket (Supabase)
// - izda račun (PDF, shrani v Storage)
// - pošlje kupcu e-pošto (QR + račun)
// - vedno vrne 200 OK Stripe-u
...
if (process.env.BREVO_API_KEY) brevo.setApiKey(Brevo.TransactionalEmailsApiApiKeys.apiKey, process.env.BREVO_API_KEY);
const FROM_EMAIL = (EMAIL_FROM.match(/<([^>]+)>/u) || [null, EMAIL_FROM])[1];
const FROM_NAME  = EMAIL_FROM.replace(/\s*<[^>]+>\s*$/u, "") || "NearGo";

/* ===== ABS poti do pisav – varno tudi v CJS bundlu ===== */
const TASK_ROOT = process.env.LAMBDA_TASK_ROOT || process.cwd(); // npr. /var/task
const FONTS_DIR = path.join(TASK_ROOT, "netlify", "functions", "fonts");
const FONT_REG_PATH  = path.join(FONTS_DIR, "NotoSans-Regular.ttf");
const FONT_BOLD_PATH = path.join(FONTS_DIR, "NotoSans-Bold.ttf");
console.log("[webhook] fonts dir:", FONTS_DIR);
console.log("[webhook] font reg:",  FONT_REG_PATH);
console.log("[webhook] font bold:", FONT_BOLD_PATH);
...
/* PDF račun — Unicode (NotoSans) — vrne Buffer */
async function createInvoicePdf({ seqNo, buyer, items, subtotal, taxRate, taxAmount, total, paidAt, sessionId }) {
  const pdf = await PDFDocument.create();
  const page = pdf.addPage([595.28, 841.89]); // A4
  const { height } = page.getSize();

  // preberi TTF kot Buffer (če manjka, log)
  if (!fs.existsSync(FONT_REG_PATH) || !fs.existsSync(FONT_BOLD_PATH)) {
    console.error("[webhook] MISSING FONT FILES in", FONTS_DIR);
    throw new Error("Missing NotoSans fonts in functions/fonts");
  }
  const fontRegBytes  = fs.readFileSync(FONT_REG_PATH);
  const fontBoldBytes = fs.readFileSync(FONT_BOLD_PATH);

  const font     = await pdf.embedFont(fontRegBytes,  { subset: true });
  ...
  line(COMPANY.name, 14, fontBold, rgb(0,0.25,0.45));
  if (COMPANY.addr)  line(COMPANY.addr);
  if (COMPANY.taxId) line(`DDV ID: ${COMPANY.taxId}`);
  if (COMPANY.reg)   line(COMPANY.reg);
  y -= 8;
  line(`Datum: ${new Date(paidAt).toLocaleString("sl-SI")}`);
  line(`Št. računa: ${seqNo}`);
  y -= 12;

  line(`Kupec: ${buyer.name || buyer.email}`, 12, fontBold);
  line(buyer.email); y -= 4;

  line("Postavke:", 12, fontBold);
  items.forEach(it => {
    const v = (it.unit_price/100);
    line(`• ${it.name} × ${it.qty} – ${v.toLocaleString("sl-SI",{style:"currency",currency:CURRENCY.toUpperCase()})}`);
  });
  y -= 6;

  line(`Osnova: ${(subtotal/100).toLocaleString("sl-SI",{style:"currency",currency:CURRENCY.toUpperCase()})}`, 11, fontBold);
  line(`DDV (${taxRate}%): ${(taxAmount/100).toLocaleString("sl-SI",{style:"currency",currency:CURRENCY.toUpperCase()})}`, 11, fontBold);
  line(`SKUPAJ: ${(total/100).toLocaleString("sl-SI",{style:"currency",currency:CURRENCY.toUpperCase()})}`, 13, fontBold, rgb(0,0.45,0.25));
  ...
  y -= 8;
  line(`Stripe session: ${sessionId}`, 9, font, rgb(0,0,0));

  const bytes = await pdf.save();      // Uint8Array
  return Buffer.from(bytes);           // vedno Buffer
}
  ...
  try {
    ...
    const { data: ticketIns, error: ticketErr } = await supa
      ...
      .insert({
        ...
        token,
        code: token,                 // če ima tabela NOT NULL na "code"
        status: "issued",
    ...
    const pdfPath = path.join("invoices", String(year), `${seq}.pdf`);
    console.log("[webhook] pdfPath:", pdfPath);
    console.log("[webhook] pdfBytes length:", pdfBytes?.length);
