...
/* ===== Absolute paths to fonts (ESM safe) ===== */
const __filename = fileURLToPath(import.meta.url);
const __dirname  = path.dirname(__filename);
const fontRegPath  = path.resolve(__dirname, "./fonts/NotoSans-Regular.ttf");
const fontBoldPath = path.resolve(__dirname, "./fonts/NotoSans-Bold.ttf");
console.log("[webhook] fontRegPath:", fontRegPath);
console.log("[webhook] fontBoldPath:", fontBoldPath);
...
async function createInvoicePdf({ seqNo, buyer, items, subtotal, taxRate, taxAmount, total, paidAt, sessionId }) {
  ...
  y -= 8;
  line(`Stripe session: ${sessionId}`, 9, font, rgb(0.4,0.4,0.4));
  ...
  try {
    ...
    const { data: ticketIns, error: ticketErr } = await supa
      ...
      .insert({
        ...
        token,
        code: token,                 // če je NOT NULL na "code"
        status: "issued",
