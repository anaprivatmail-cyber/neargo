...
async function load(){
  ...
  try{
    ...
    // derive points/credits from ledger entries (simple): show last known balances if provided in meta
    // The ledger endpoint may not return balances; present recent rows
    const rows = j.rows || [];
    historyEl.innerHTML = rows.length ? rows.slice(0,30).map(r=>`<div>${new Date(r.created_at||r.inserted_at).toLocaleString()} — ${r.kind} — pts ${r.points_delta} — cents ${r.credits_cents_delta} — ${r.reason||''}</div>`).join('') : 'Brez zgodovine.';
...
btnConvert.addEventListener('click', async ()=>{
  ...
  try{
    const res = await fetch('/.netlify/functions/rewards-convert',{ method:'POST', headers:{ 'content-type':'application/json' }, body: JSON.stringify({ points }) });
    const j = await res.json();
...
btnRedeem.addEventListener('click', async ()=>{
  ...
  try{
    ...
    const res = await fetch('/.netlify/functions/rewards-redeem',{ method:'POST', headers:{ 'content-type':'application/json' }, body: JSON.stringify({ type, amount_cents }) });
    const j = await res.json();
