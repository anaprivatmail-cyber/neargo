...
export const handler = async (event) => {
  try {
    ...
    if (!STRIPE_SECRET_KEY || !STRIPE_SECRET_KEY.startsWith("sk_")) {
      return json({ ok:false, error:"Stripe key missing" });
    }
    ...
    if (type === "coupon") {
      ...
      lineItems = rawLineItems.map(it=>({
        currency: it.currency || "eur",
        name: it.name || "Vstopnica",
        description: it.description || "",
        amount: Math.round(Number(it.amount) * 100), // <- cente!
        quantity: it.quantity || 1,
        image: it.image || ""
      }));
      if (lineItems.some(it => !(Number(it.amount) >= 50))) {  // min 0,50 €
    ...
    if (!res.ok || !data?.url) {
      return json({ ok:false, error: data?.error?.message || "Stripe error", debug:data }, 200);
    }
