...
// ➜ ključ za skener (če je nastavljen, ga mail doda kot ?k=)
...
export const handler = async (event) => {
  ...
  try{
    ...
    // -------- e-pošta organizatorju (logo + barve, gumbi + plaintext fallback) --------
    ...
    if (!BREVO_API_KEY) {
      ...
      try{
        ...
        // table-based bulletproof button + plaintext fallback (vedno vidno in klikljivo)
        const btn = (url, label) => `
          <table role="presentation" cellspacing="0" cellpadding="0" border="0" align="left" style="margin:0 0 6px 0">
            <tr>
              <td align="center" bgcolor="${primary}" style="border-radius:999px;">
                <a href="${url}" target="_blank" rel="noopener noreferrer"
                   style="display:inline-block;padding:12px 18px;font-weight:800;color:#ffffff;text-decoration:none;">
                  ${label}
                </a>
              </td>
            </tr>
          ...
          <div style="font-size:12px;color:#6b7280;margin-top:6px;line-height:1.4">
            Če gumb ne deluje, kopirajte ta naslov:<br>
            <a href="${url}" style="color:#0b1b2b;word-break:break-all;text-decoration:underline" target="_blank" rel="noopener noreferrer">
              ${url}
            </a>
          </div>`;
        ...
        <div style="font-family:Inter,system-ui,Segoe UI,Roboto,Arial,sans-serif;max-width:680px;margin:auto;border:1px solid #e3eef7;border-radius:12px;overflow:hidden">
          ...
          <div style="padding:18px 20px;color:#0b1b2b">
            ...
            <p style="margin:14px 0 8px 0;font-weight:800">Urejanje prijave</p>
            ${btn(editLink, "TUKAJ")}

            <p style="margin:18px 0 8px 0;font-weight:800">Pregled prodaje & skeniranje QR</p>
            ${btn(dashLink, "ODPRI PREGLED")}

            <p style="margin:18px 0 0;color:#5b6b7b;font-size:13px">
              Če povezavi ne delujeta, kopirajte ju v brskalnik. Ta e-pošta je bila poslana samodejno.
            </p>
          </div>
        </div>`;
