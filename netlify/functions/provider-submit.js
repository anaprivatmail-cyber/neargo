  ...
  try{
    ...
    try {
      if (BREVO_API_KEY && payload.organizerEmail) {
        const contact   = escapeHtml(payload.organizerFullName || payload.organizer || ''); // ← sprememba (ime in priimek)
        ...
        const html = `
          <div style="font-family:Arial,Helvetica,sans-serif;max-width:680px;margin:auto;border:1px solid #e3eef7;border-radius:12px;padding:16px">
            ...
            <p><b>Uredi dogodek:</b> <a href="${linkEdit}">${linkEdit}</a></p>
            <p><b>Statistika prodaje/unovčitev:</b> <a href="${linkStats}">${linkStats}</a></p>
