...
export const handler = async (event) => {
  try {
    ...
    const statsToken = (qs.stats || "").trim();       // žeton iz potrditvenega maila
    const eventIdQ   = (qs.eventId || "").trim() || null;

    // DOSTOP: EITHER stats token OR scanner key
    ...
    if (statsToken) {
      // Organizator dostopa prek stats tokena (dogodek iz tabele events)
      ...
      // Skener dostopa z eventId ali globalno (če eventId ni podan)
      if (eventIdQ) {
        const { data: ev } = await supa.from("events").select("id, title, city, start").eq("id", eventIdQ).single();
        if (!ev) return json({ ok:false, error:"unknown_event" },404);
        eventFilter = { id: ev.id, meta: { title: ev.title, city: ev.city, start: ev.start } };
      } else {
        // globalni pregled za skenerja
        eventFilter = null;
      }
    } else {
    ...
    // SOLD = vsi izdani zapisi (status 'issued' ali 'redeemed' – robustno na velikost črk)
    const sold = await countTickets(whereEvent, ["issued","ISSUED","redeemed","REDEEMED"]);

    // REDEEMED = status 'redeemed' (robustno)
    const redeemed = await countTickets(whereEvent, ["redeemed","REDEEMED"]);

    // Zadnja prodaja (če imaš stolpec created_at)
    const lastSale = await getLastSaleAt(whereEvent);

    // Razbitje po tipu (če stolpec 'type' obstaja)
...
async function getLastSaleAt(whereEvent){
  ...
  let q = supa.from("tickets").select("created_at", { head:false }).order("created_at", { ascending:false }).limit(1);
  if (whereEvent) q = q.eq(whereEvent.column, whereEvent.value);
...
async function breakdownByType(whereEvent){
  // Poskusi group by 'type' in status (če stolpca ni, vrni prazen objekt)
  try{
    // vzemi minimalni sample, da preverimo obstoj stolpca
    ...
    if (!p || typeof p[0]?.type === "undefined") return {}; // stolpec ne obstaja
