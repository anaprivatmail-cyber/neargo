...
async function createInvoicePdf({ seqNo, buyer, totalGross, base, tax, paidAt, sessionId }) {
  ...
  if (COMPANY_REGISTER_INFO || COMPANY_SHARE_CAPITAL) {
    y -= 14;
    if (COMPANY_REGISTER_INFO) line(COMPANY_REGISTER_INFO, 9, F_REG, rgb(0.35,0.35,0.35));
    if (COMPANY_SHARE_CAPITAL) line(COMPANY_SHARE_CAPITAL, 9, F_REG, rgb(0.35,0.35,0.35));
  }
...
exports.handler = async (event) => {
  try {
    ...
    try {
      ...
      // 200 + received:true, da Stripe ne retry-a v nedogled
    ...
    // Upload QR (v isti bucket: invoices/passes/qr/<token>.png)
    try {
      const qrPath = `passes/qr/${token}.png`;
      await supa.storage.from("invoices").upload(qrPath, qrPngBuffer, {
    ...
    // Insert v invoices in signed URL
    ...
    if (process.env.BREVO_API_KEY && customerEmail) {
      ...
      // DODANO: pravna opomba za kupone
      ...
      const html = `
        <div style="font-family:Arial,Helvetica,sans-serif;max-width:680px;margin:auto;border:1px solid #e3eef7;border-radius:12px;overflow:hidden">
          ...
          <div style="padding:18px 20px;color:#0b1b2b">
            ...
            <p style="margin:0 0 6px">Koda: <b>${token}</b><br><small>Namig: v aplikaciji NearGo odpri “Moje”, izberi “Dodaj kodo” in prilepi zgornjo kodo za hiter dostop do QR tudi offline.</small></p>
