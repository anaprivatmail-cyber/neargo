...
export const handler = async (event) => {
  ...
  let type = "ticket";
  if (md.type === "coupon") type = "coupon";
  if (md.type === "premium") type = "premium";
  if (md.type === "provider-plan") type = "provider-plan";

  // PREMIUM EMAIL
  if (type === "premium" && customerEmail && process.env.BREVO_API_KEY) {
      const logoTargetSvg = `
        ...
        <div style="font-family:Arial,Helvetica,sans-serif;background:#f6fbfe;padding:0;margin:0">
          <div style="max-width:680px;margin:0 auto;border:1px solid #e3eef7;border-radius:14px;overflow:hidden;background:#fff">
            ...
            <div style="padding:20px 22px;color:#0b1b2b">
              <h2 style="margin:0 0 12px 0;font-size:20px;line-height:1.35">Hvala za nakup NearGo Premium!</h2>
              <p style="font-size:16px;margin-bottom:10px;color:#0bbbd6;font-weight:700">Vaš Premium je aktiviran. Uživajte v vseh prednostih!</p>
              <ul style="margin:12px 0 18px 0; color:#0bbbd6; font-size:15px">
                <li>Predčasna obvestila, ekskluzivne ponudbe, napredni filtri, brez oglasov, VIP podpora</li>
                <li>Predlagajte nove funkcije ali izboljšave: <a href="mailto:info@getneargo.com">info@getneargo.com</a></li>
                <li>Zbirajte točke z uporabo aplikacije in povabili prijateljev</li>
                <li>Vse vaše vstopnice, kuponi in izbrani dogodki so v razdelku <a href="${PUBLIC_BASE_URL}/my.html" style="color:#0bbbd6;font-weight:800">Moje</a></li>
              </ul>
              <div style="margin:18px 0 4px;color:#5b6b7b;font-size:13px">
                Vprašanja? <a href="mailto:info@getneargo.com" style="color:#0bbbd6;font-weight:800">info@getneargo.com</a>
              </div>
              <div style="margin-top:18px;font-size:13px;color:#0bbbd6;font-weight:700">Ekipa NearGo</div>
            </div>
          </div>
        </div>`;
    // PROVIDER PLAN EMAIL (Grow/Pro)
    if (type === "provider-plan" && customerEmail && process.env.BREVO_API_KEY) {
      const logoTargetSvg = `
        <svg width="36" height="36" viewBox="0 0 32 32" xmlns="http://www.w3.org/2000/svg">
          <circle cx="16" cy="16" r="13" fill="none" stroke="#0b1b2b" stroke-width="2"/>
          <circle cx="16" cy="16" r="8"  fill="none" stroke="#0b1b2b" stroke-width="2" opacity="0.9"/>
      ...
      const html = `
        <div style="font-family:Arial,Helvetica,sans-serif;background:#f6fbfe;padding:0;margin:0">
          <div style="max-width:680px;margin:0 auto;border:1px solid #e3eef7;border-radius:14px;overflow:hidden;background:#fff">
            ...
            <div style="padding:20px 22px;color:#0b1b2b">
              <h2 style="margin:0 0 12px 0;font-size:20px;line-height:1.35">Hvala za nakup paketa za ponudnike!</h2>
              <p style="font-size:16px;margin-bottom:10px;color:#0bbbd6;font-weight:700">Vaš paket je aktiviran. Uživajte v vseh prednostih za organizatorje!</p>
              <ul style="margin:12px 0 18px 0; color:#0bbbd6; font-size:15px">
                <li>Napredna analitika, več dogodkov, promocija, VIP podpora</li>
                <li>Predlagajte nove funkcije ali izboljšave: <a href="mailto:info@getneargo.com">info@getneargo.com</a></li>
                <li>Vse vaše dogodke, prodaje in statistike najdete v razdelku <a href="${PUBLIC_BASE_URL}/organizers.html" style="color:#0bbbd6;font-weight:800">Organizator</a></li>
              </ul>
              <div style="margin:18px 0 4px;color:#5b6b7b;font-size:13px">
                Vprašanja? <a href="mailto:info@getneargo.com" style="color:#0bbbd6;font-weight:800">info@getneargo.com</a>
              </div>
      ...
      try {
        ...
        email.subject     = "NearGo paket – potrdilo";
        email.htmlContent = html;
        if (typeof pdfBytes !== "undefined" && pdfBytes) {
          email.attachment = [
            { name: `Racun-Paket.pdf`, content: Buffer.from(pdfBytes).toString("base64") }
          ];
        }
        await brevoApi.sendTransacEmail(email);
      } catch (mailErr) {
        console.error("[webhook] Brevo send error (provider-plan):", mailErr?.message || mailErr);
      }
      return { statusCode:200, headers:cors(), body: JSON.stringify({ ok:true, providerPlan:true }) };
    }
      try {
        const email = new Brevo.SendSmtpEmail();
        email.sender      = { email: FROM_EMAIL, name: FROM_NAME };
        email.to          = [{ email: customerEmail }];
        email.subject     = "NearGo Premium – potrdilo";
        email.htmlContent = html;
        if (typeof pdfBytes !== "undefined" && pdfBytes) {
          email.attachment = [
            { name: `Racun-Premium.pdf`, content: Buffer.from(pdfBytes).toString("base64") }
          ];
        }
        await brevoApi.sendTransacEmail(email);
      } catch (mailErr) {
        console.error("[webhook] Brevo send error (premium):", mailErr?.message || mailErr);
      }
      return { statusCode:200, headers:cors(), body: JSON.stringify({ ok:true, premium:true }) };
    }
